...
class Ingredient(Base):
    ...
    # Cost per unit (e.g., Â£0.006 per gram)
    ...
    pack_cost = Column(Float, nullable=True)  # e.g., Â£3.00
...
def seed_initial_data():
    ...
    try:
        ...
        if db.query(Person).count() == 0:
            ...
            db.add_all(people)
            db.commit()
            print("âœ… Added people: Michael, Lorna, Izzy")
        
        print("âœ… Database seeded")
        
    except Exception as e:
        print(f"âŒ Error seeding data: {e}")
        db.rollback()
    finally:
        db.close()
